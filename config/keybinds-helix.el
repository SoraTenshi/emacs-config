...
(defun helix-forward-word-no-space ()
  ...
  (helix--with-movement-surround
   (when (re-search-forward "\\([[:alnum:]]+\\)\\|\\([[:punct:]]+\\)\\|\n" nil 'move)
...
(defun helix-select-line-up ()
  "Select the current line, extending upward on subsequent calls."
  (interactive)
...
(with-eval-after-load 'helix
  ...
  (helix-define-key 'space "w" #'evil-delete-buffer)
  (helix-define-key 'space "d" #'ui/diagnostic-list)
  (helix-define-key 'space "-" #'ui/diagnostic-list)
  (helix-define-key 'space "p" #'project/switch)
  (helix-define-key 'space "D" #'ui/project-diagnostic-list)
  (helix-define-key 'space "B" #'buffer/centaur-switch-to-group)
  (helix-define-key 'space "W" #'ui/centaur-tabs-remove-group)
  (helix-define-key 'space "D" #'ui/project-diagnostic-list)
  (helix-define-key 'space "D" #'ui/project-diagnostic-list)
  (helix-define-key 'space "R" #'clipboard/replace-region)

  (helix-define-key 'goto "r" #'xref-find-references)
